...
const ObserverWorksheet: React.FC = () => {
  useDocumentTitle('Observer Worksheet');
  const [form, setForm] = React.useState<ObserverForm>(() => {
    try {
      return JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}') as ObserverForm;
    } catch {
      return {};
    }
  });
  ...
  const clear = () => {
    if (!confirm('Clear all saved inputs for Observer worksheet?')) return;
    localStorage.removeItem(STORAGE_KEY);
    setForm({});
  };
  const onChange = (k: keyof ObserverForm) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) =>
    setForm((f) => ({ ...f, [k]: e.target.value }));

  const protocolSections: Array<{ key: keyof ObserverForm; title: string; prompts: string[] }> = [
    {
      key: 'protocolListen',
        ...
        >
          <Stack spacing={1.5}>
            ...
            <Typography color="text.secondary" sx={{ fontSize: 14, maxWidth: 720 }}>
              The process observer’s role is to observe a meeting while participating, and give feedback for everybody to learn from good examples and to improve the quality of a meeting.
            </Typography>
          </Stack>
        </Paper>
          ...
          <Stack spacing={3}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                (E.g.: “The question XXX–san asked really helped the presenter see the issue from the positive side.”; “The presenter shared a very tough issue with courage. It helped other members to speak deeper from the heart.”)
              </Typography>
              <TextField
                fullWidth
                multiline
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                (E.g.: “When YYY–san asked a question about ‘(specific topic)‘, it may have sounded like criticizing.”; “When sharing experiences, QQQ–san told the presenter ‘you should do this,’ but probably without realizing.”)
              </Typography>
              <TextField
                fullWidth
                multiline
        ...
        >
          <Stack spacing={2.5}>
            ...
            <Typography variant="body2" color="text.secondary">
              Review each step and capture concrete observations that can help the team reinforce strengths and improve future sessions.
            </Typography>
...
export default ObserverWorksheet;
